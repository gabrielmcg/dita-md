...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="li" mode="ast">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:message>***<xsl:value-of select="$label"/>***<xsl:value-of select="substring($default-indent, string-length($label) + 1)"/>***</xsl:message>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="image" mode="ast" priority="100">
    <xsl:text>![</xsl:text>
